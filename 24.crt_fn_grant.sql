/*
Usage : psql -d {DBNAME} -t -f 24.crt_fn_grant.sql > /home/gpadmin/gpconfigs/fn20230808/{DBNAME}/24.crt_fn_grant.{DBNAME}.sh
*/

SELECT 'echo ' || '"' || 'GRANT ALL ON FUNCTION ' || fl.nspname || '.' || fl.proname || '(' || fl.arg_val2 || ') TO ' || fg.grantee || ';' || '" ' || '>> ' || :val || '/' || fl.dbname || '/' || fl.nspname || '.' || fl.proname || '_' || array_to_string(fl.arg_val,',')
FROM
	(SELECT st.dbname
	, st.nspname
	, st.proname
